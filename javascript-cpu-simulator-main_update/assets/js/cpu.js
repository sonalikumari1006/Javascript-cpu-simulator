...
class CPU {
    // 8-bit CPU Architecher
    constructor() {
        this.cpuData = {
            cpuEnabled: false,
            clockState: 0,
            memoryArray: [],
            programCounter: 0xFFFE, // 16 bit register
            registers: {
    ...
    convertOnDecimalMode(number) {
        if(this.cpuData.flags.decimal){
            ...
            var firstDigit  = strNumber.substr(0,1); // string.substr(start, length)
    ...
    updateEnabled(enabled) {
        ...
        self.clock.style.color = '#800000'; //  dark red color
    }
    ...
    clockTick() {
        if (cpu.cpuData.cpuEnabled) {
            if (cpu.cpuData.clockState == 1) {
                self.clock.style.color = '#FF0000'; // light red
                cpu.loaderRun();
                cpu.cpuData.clockState = 0;
            }
            else {
                self.clock.style.color = '#800000'; // dark red
    ...
    // Assembly code (or assembly language) is a low-level programming language that is closely related to the machine code used by a computer's processor (CPU).
    // Each CPU may have different assemble code
    assembleCode() {
        // halt cpu for assembly
        this.cpuData.cpuEnabled = self.cpuEnabled.checked = false;

        // clear existing memory
        this.clearAndInitializeMemory(65536); //  65536 byte =64KB of memory
        ...
        lines.forEach(line => {
            if (line.trim() != "") {
                ...
                if (line.trim().startsWith('.')) {
                    ...
                    // console.log(directiveParts)
                    var directiveCode = directiveParts[0].trim().toUpperCase();
                    // console.log(directiveCode)
                                ...
                                case 'INC':
                                    ...
                                    switch (operandType) {
                                        case operandTypes.ABSOLUTE:
                                            ...
                                            nextMemoryIndex++;
                                            this.cpuData.assembler.privatePointer += 3;
                                            // console.log("Execute INC in absolute mode ")
                                            break;
                                        case operandTypes.NULL:
                                            // console.log("Execute INC in null mode")
    ...
    loaderRun() {
        ...
        switch (operation) {
            ...
            case 0xA0:
                ...
                this.cpuData.flags.negative = (sum > 0x7F);
                this.cpuData.registers.Y    = sum;
                console.log("LDY: PC= ",this.cpuData.programCounter);
                this.cpuData.programCounter += 2;
                console.log("LDY: PC= ",this.cpuData.programCounter);
                break;
